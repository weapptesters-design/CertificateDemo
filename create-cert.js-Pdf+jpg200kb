...
// ─── JPG from PDF via Ghostscript ────────────────────────────────────────────
function makeJPG(pdfPath, jpgPath) {
  execSync(
    'gs -sDEVICE=jpeg -dJPEGQ=92 -r150' +
    ' -dNOPAUSE -dQUIET -dBATCH' +
...
function tgSendFile(chatId, caption, fileBuffer, fileName, contentType) {
  ...
  for (let i = 0; i < newApps.length; i++) {
    ...
    try {
      ...
      try {
        ...
        await tgSendFile(CHAT_ID, app.appName + ' (Image)', jpgBuf, jpgName, 'image/jpeg');
